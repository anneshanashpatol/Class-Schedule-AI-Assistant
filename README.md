...
助手的身份、对话原则和权限边界写在 `server/COURSE_ASSISTANT.md`，构建时生成供 Worker 使用的 TypeScript 常量。`server/skills.ts` 统一登记十四项能力及适用角色；明确意图会缩小本轮模型提示，模糊或多项目的仍交给模型判断，再由服务端验证操作、查询真实目标。模型只生成自然语言回复、追问和结构化操作，不直接写业务数据。页面记住当前会话中的最近对话，刷新后清空；待确认操作仍可短期存在助手 D1 中以防重复执行。
