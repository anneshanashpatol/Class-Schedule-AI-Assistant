...
对硅基流动 `THUDM/GLM-4-9B-0414`，助手优先使用原生 `tools` 参数提交操作计划；模型只给出要使用的能力和筛选字段，Worker 执行真实查询与确认流程。已用真实接口验证该模型能返回完课工具调用。其他兼容模型沿用结构化 JSON。若硅基流动端拒绝 `tools` 参数，当次请求回退一次到 JSON，此 Worker 实例随后跳过原生工具尝试。正常操作请求仍只需一次模型调用，不做查询后的第二轮模型生成；常见课程问答直接走自然语言回复，不重新触发上一轮写操作。
